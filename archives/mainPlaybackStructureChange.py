        ...
        if data.activeScreen == 'openPDF':
            for file in data.fileList:
                ...
                if x1 < event.x < x2 and y1 < event.y < y2:
                    ...
                    data.loading = True
                    data.timeLoad = 20000 # in milliseconds
                    data.loadpoint = 0
                    data.loadText = 'Loading.'
                    ###################
...
def loadingScreen(canvas, data):
    ...
    loadWidth = 300
    loadHeight = 200
    data.loadpoint += 100 # load point increases every 100 ms
    
    if data.loading == True:
        canvas.create_rectangle(data.width/2 - loadWidth/2,
                                data.height/2 - loadHeight/2,
                                data.width/2 + loadWidth/2,
                                data.height/2 + loadHeight/2,
                                fill=color(80, 80, 80),
                                width=0)
        print('data.loadpoint is', data.loadpoint)
        if data.loadpoint % 3000 == 0:
            data.loadText = 'Loading.'
        elif data.loadpoint % 2000 == 0:
            data.loadText = 'Loading...'
        elif data.loadpoint % 1000 == 0:
            data.loadText = 'Loading..'
...
def timerFired(data):
    if data.activeScreen == 'play' and data.track != []:
        ...
        if data.loading == True:
            data.timeLoad -= 100 # in milliseconds

            if data.timeLoad < 100:
                data.loading = False # stop the loading once 20s is up
